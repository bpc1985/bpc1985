# Hi there, I'm [![Hung Ho](https://img.shields.io/badge/Website-333333?style=for-the-badge&logo=globe&logoColor=white)](https://bpc1985.github.io/)
 👋

**Full-Stack Developer** based in Espoo, Finland 🇫🇮

I build scalable, high-performance web applications with a focus on clean code, modern architectures, and great user experiences.

## 🚀 What I'm Working On

- Full-stack Developer building intelligent automation tools and web applications

## 💼 Professional Experience

- **Full-stack Developer** at ResultElf Oy (2021–2024)
- **Full-stack Developer** at Quanturi Oy (2020–2021)
- **Frontend Developer** at Verto Analytics Oy (2014–2019)

## 🛠️ Tech Stack

**Frontend**
```
TypeScript • React • Next.js • Vue.js • TailwindCSS • Redux • Zustand
```

**Backend**
```
Node.js • NestJS • Express • FastAPI • REST APIs • GraphQL
```

**Databases**
```
PostgreSQL • MongoDB • Prisma
```

**DevOps & Cloud**
```
Docker • Kubernetes • GitHub Actions • AWS (Lambda, RDS, CloudWatch, DynamoDB)
```

**Additional**
```
Python • Golang • Java • React Native • Flutter
```

## 🤝 Let's Connect
[![Website](https://img.shields.io/badge/Website-333333?style=for-the-badge&logo=globe&logoColor=white)](https://bpc1985.github.io/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/hongochung)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hungho2812@gmail.com)

---

<p align="center">
  <i>Open to remote or hybrid opportunities in Finland and the EU</i>
</p>
